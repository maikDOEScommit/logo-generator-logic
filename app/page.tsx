    ...
    <>
      ...
      <main className={`relative w-full grid grid-cols-1 ${expandPreviewPanel ? 'md:grid-cols-1' : (showPreviewPanel && !visibleSections.includes(4)) ? 'md:grid-cols-2' : 'md:grid-cols-1'} pt-20 transition-all duration-500`} style={{ minHeight: '100vh' }}>
        <div className={`p-4 md:p-8 lg:p-12 overflow-y-auto ${expandPreviewPanel ? 'hidden' : (showPreviewPanel && !visibleSections.includes(4)) ? '' : 'md:col-span-1'} transition-all duration-500`}>
          ...
          {/* Marketing Section - Moved after Hero/CTA */}
          {!hideNewSection && (
            <motion.section
              ...
              className="relative py-20 px-4 md:px-8 lg:px-12 overflow-hidden mt-8"
              style={{
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  ...
                  >
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      onClick={() => setHideNewSection(true)}
                        ...
                        {fontCategories.map((category) => (
                          ...
                          >
                            ...
                            <div className={`text-xs leading-relaxed ${isDarkMode ? 'text-black/60' : 'text-white/60'}`}>
                              ...
                              {category.name === 'Timeless' && 'Classic, enduring fonts for established brands'}
                              {category.name === 'Playful' && 'Fun, creative fonts for vibrant businesses'}
                              {category.name === 'Tech' && 'Futuristic, digital fonts for tech startups'}
                            </div>
                          </button>
                        ))}
                      </div>
                          ...
                          {showThirdText && !exitThirdText && (
                            ...
                            >
                              ...
                              <motion.span 
                                ...
                                exit={{ x: '100%', opacity: 0 }}
                                transition={{ duration: 0.5, delay: 0.2 }}
                                className="block text-gray-600 my-4"
                              >
                                we&apos;re right
                              </motion.span>
                              <motion.span 
      ...
      {/* Removed old marketing section - moved to after Hero/CTA */}
      ...
      {/* Removed duplicate marketing section */}
      {false && (
        <motion.section
          initial={{ opacity: 1, height: 'auto' }}
          animate={{
          ...
          style={{
            background: 'linear-gradient(135deg, rgba(248, 250, 252, 0.8) 0%, rgba(226, 232, 240, 0.8) 100%)' // 80% transparent
          }}
        >
          {/* Organic Curve Background */}
          <div className="absolute inset-0 overflow-hidden">
